...
# Возвращает план выполнения PostgREST (требует PostgREST ≥ 11)
```
...
- **Только чтение** — создание, обновление и удаление не реализованы. PostgREST поддерживает операции записи, но эта библиотека предназначена только для чтения.
...
**Условия OR / AND** — AR использует `.or(Model.where(...))`, здесь отдельные методы:
...
- **Мутации** — нет `create`, `update`, `destroy`, `save`, `upsert`. PostgREST поддерживает операции записи; библиотека в настоящее время только для чтения.
